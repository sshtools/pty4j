...
public class PtyUtil {
	...
	public static String[] toStringArray(Map<String, String> environment) {
		if (environment == null)
			return new String[0];
		List<String> list = Lists.transform(Lists.newArrayList(environment.entrySet()),
	...
	public static String getPtyLibFolderPath() throws Exception {
		...
		// Class aclass =
		// WinPty.class.getClassLoader().loadClass("com.jediterm.pty.PtyMain");
	...
	public static File resolveNativeLibrary() throws Exception {
		...
		if (libFolderPath != null) {
			...
			File nativeLib = resolveNativeLibrary(libFolder);
			File lib;
			if (nativeLib.exists())
				lib = nativeLib;
			else {
				nativeLib = resolveNativeLibrary(new File(libFolder, "libpty"));
				if (nativeLib.exists())
					lib = nativeLib;
				else {
					libFolder = new File(libFolderPath + "/os");
					nativeLib = resolveNativeLibrary(libFolder);
					lib = nativeLib;
				}
			}
			if (!lib.exists()) {
				throw new IllegalStateException(String.format("Couldn't find %s, jar folder %s or %s", lib.getName(),
						libFolder.getAbsolutePath(), nativeLib.getAbsolutePath()));
			}
			return lib;
		} else {
